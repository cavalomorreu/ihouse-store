...
import {openModalQuickView, openLongModal, openDefaultModal, openCustomModal} from "../../functions/modal";
...
import {slickGallery, destroyCarousel} from "../../ui/modules/_carGallery.js";
...
$(document).ready(function () {
	...
	$(".button.avise").api({
		...
		onSuccess: function (response) {
			...
			openCustomModal($("#form-alert"), settings_modal, "data-ref-id");
		},
	...
	$(document).on("click", ".infoProduto .avaliacoes", function(e){
...
$(document).on("click", "#guiaMedidas", function () {
	openDefaultModal($("#guiaMedidasModal"));
});
...
function VariacaoDropDown() {
	$('.product-grid .dropdown').dropdown({
		onChange: function (value, text, selectedItem) {
			...
			buscarSKU('principal');
			//$(this).dropdown('set selected', value); //$('.product-grid .dropdown').dropdown('set selected', value);
		}
	});
} 
...
function BuscarVariacaoCor(valor_selecionado, order, idReferencia) {
	$.ajax({
		...
		success: function (response) {
			...
			$(".imagensProduto").addClass(isMobile() ? "viewMobile" : "viewDesktop");
			slickGallery();
...
function AtualizarGrade(jsonSKU) {
	...
	if (estoque == 0) {
		...
		//$(".btn-comprar").addClass("disabled").html("<div class=\"visible content\">PRODUTO ESGOTADO</div>");
		...
		//$(".btn-comprar").removeClass("disabled").html("COMPRAR");
...
function MontaTipoCheckBox(skU, seletor_produto, reference, variation, variacoes_selecionadas_bkp, flag_buscar_sku, posicao) {
	var html      = "";
	var classeBtn = "ui basic secondary custom button radio tiny variacao-radio";
