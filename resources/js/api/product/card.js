...
$(document).ready(function () {
	...
	if ($(".conjunct_product:checked").length > 0) {
		...
		$(".buy-conjunct").addClass("ui button fluid grey disabled")
			...
			success: function (response) {
				...
				if (sku.Price > 0 && sku.PricePromotion > 0) {
					if (ponteiroPricesCurrent > 0) {
						...
						$parent.find(".preco").before('<span id="basePrice_' + productId + '" class="precoBase"><i>' + moneyPtBR(sku.Price) + '</i></span>');
					}
				...
				if (sku.Stock <= 0 || isExhausted(productId)) {
					...
					*/
					...
					$parent.find("#btn-comprar-card-" + productId)
						...
						.addClass("btn-comprar-card")
						.html('<i class="icon add to cart hideme"></i> Comprar');

					$parent.find("#add-event-list-" + productId)
						.removeClass("grey avise-card avise-me-modal")
						.addClass("ui button add-event-list")
						.html('<i class="icon add to cart hideme"></i> Adicionar à Lista');
